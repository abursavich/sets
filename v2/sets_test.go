...
var (
	_ viewer[string]    = (*table[string])(nil)
	_ tableView[string] = (*table[string])(nil)

	_ viewer[string]   = (*ordered[string])(nil)
	_ listView[string] = (*ordered[string])(nil)

	_ viewer[string]   = (*funcSorted[string])(nil)
	_ listView[string] = (*funcSorted[string])(nil)
)
...
func TestOrderedSets(t *testing.T) {
	newSetTester(t, []rune("abcdefghijklmnop"), []*setType[rune]{
		...
		{
			name:    "ordered",
			newSet:  func(elems ...rune) Mutable[rune] { return NewSortedMutable(elems...).Mutable() },
			cmpFn:   cmp.Compare[rune],
			eqFn:    equal[rune],
			sorted:  true,
			uniqCmp: true,
		...
		{
			name:    "sorted",
			newSet:  func(elems ...rune) Mutable[rune] { return NewSortedMutableFunc(cmp.Compare[rune], elems...).Mutable() },
			cmpFn:   cmp.Compare[rune],
			eqFn:    equal[rune],
			sorted:  true,
			uniqCmp: true,
		...
		{
			name:   "external",
			newSet: func(elems ...rune) Mutable[rune] { return &externalSet[rune]{NewMutable(elems...)} },
			skip:   true,
		},
	}).test(t)
}
...
func TestUnorderedSets(t *testing.T) {
	newSetTester(t, toRunePtrs("aaabbbcccdddeee"), []*setType[*rune]{
		...
		{
			name: "sorted",
			newSet: func(elems ...*rune) Mutable[*rune] {
				return NewSortedMutableFuncs(cmpPtrVal[rune], equal[*rune], elems...).Mutable()
			},
			cmpFn:   cmpPtrVal[rune],
			eqFn:    equal[*rune],
			sorted:  true,
...
func (st *setTester[E]) check(t *testing.T, typ *setType[E], set Mutable[E], elems []E) {
	t.Helper()
